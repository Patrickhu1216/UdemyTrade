...
- 回測執行細節： 結合 Z-Score 訊號 + 波動率濾網，計算每日部位價值。

- 動態風險指標（比看單一數值更重要）：
    - 滾動夏普值 (Rolling Sharpe Ratio)： 觀察策略的「風險報酬比」是否在不同年份都能維持穩定
     （例如：不能只有 2020 年很高，其他年份都很低）。

    - 滾動最大回撤 (Rolling Max Drawdown)： 掌握策略在任何連續期間內，可能面臨的最大虧損幅度。
